...
class CSRM:
    ...
    def train(self, Train_data, Validation_data, Test_data, result_path='save/'):
        ...
        for epoch in range(self.epoch):
            ...
            if epoch % 5 == 0:
                ...
                for k in range(len(Ks)):
                    ...
                    valid_evaluation, _ = self.pred_evaluation(Validation_data, kf_valid, session_memory_state, k = Ks[k])
                    hit[k] = valid_evaluation[0]
                    MRR[k] = valid_evaluation[1]
                    cov[k] = valid_evaluation[2]
                    pop[k] = valid_evaluation[3]
                with open("LOGGER_"+ self.expname + ".txt", "a") as myfile:
                    myfile.write("**************************************************\n")
            ...
            print("This is the total_loss --->  ", np.mean(epoch_loss))
        ...
        logger.debug('Final result is %g', np.mean(epoch_loss))
        logger.debug('Send final result done.')


        #memory_task.kill()
        return hit, MRR, cov, pop, train_time, test_time/len(Ks)
