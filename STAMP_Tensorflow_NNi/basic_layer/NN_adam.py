...
class NN(object):
    ...
    def save_model(self, sess, config, saver=None):
        suf = "ModelCheckpoint-" + time.strftime("%Y%m%d%H%M", time.localtime())
        if saver is not None:
            saver.save(sess, config['model_save_path'] + "model_ckpt.ckpt-" + suf)
        config['saved_model'] = config['model_save_path'] + "model_ckpt.ckpt-" + suf
