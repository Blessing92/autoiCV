...
class Seq2SeqAttNN(NN):
    ...
    def build_model(self):
        ...
        self.sequence_length = tf.placeholder(tf.int64, [None], name='sequence_length')
        self.lab_input = tf.placeholder(tf.int32, [None], name="lab_input")
        # the lookup dict.
        self.embe_dict = tf.Variable(self.pre_embedding, dtype=tf.float32, trainable=self.emb_up)
        self.pe_mask = tf.Variable(self.pre_embedding_mask, dtype=tf.float32, trainable=False)
        ...
        pool_out = pooler(org_memory, 'mean', axis=1, sequence_length = tf.cast(tf.reshape(self.sequence_length,[batch_size, 1]), tf.float32))
        pool_out = tf.reshape(pool_out,[-1,self.hidden_size])

        attlayer = FwNnAttLayer(self.edim, active=self.active, stddev=self.stddev, norm_type='none')
        ...
        self.w1 = tf.Variable(tf.random_normal([self.edim, self.edim], stddev=self.stddev), trainable=True)
        self.w2 = tf.Variable(tf.random_normal([self.edim, self.edim], stddev=self.stddev), trainable=True)
    ...
    def train(self,sess, train_data, test_data=None, saver = None, threshold_acc=0.5):
        max_recall = 0.0
        max_mrr = 0.0
        for epoch in range(self.nepoch):   # epoch round.
            batch = 0
            c = []
            bt = batcher(samples=train_data.samples, class_num= self.n_items, random=True)
            ...
            if test_data != None:
                ...
                test_data.flush()
        if self.is_print:
            TIPrint(test_data.samples, self.config, {'recall': max_recall, 'mrr': max_mrr}, True)
            ...
            if batch_lenth > self.batch_size:
                ...
                if remain > 0:
                    ...
                    for s in range(len(tmp_in_data[0])):
                        ...
                        # train
                        preds, loss, alpha = sess.run([self.softmax_input, self.loss, self.alph], feed_dict=feed_dict)
                        t_r, t_m, ranks = cau_recall_mrr_org(preds, batch_out, cutoff=self.cut_off)
                        test_data.pack_ext_matrix('alpha', alpha, tmp_batch_ids)
                        test_data.pack_preds(ranks, tmp_batch_ids)
                        c_loss += list(loss)
